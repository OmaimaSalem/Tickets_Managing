...
if(!function_exists('htmLawed')) {
    function htmLawed($t, $C=1, $S=array()){
        ...
        // final $spec
        $s = array();
        $t = preg_replace('`\s+`', ' ', preg_replace_callback(array('`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'), function ($m) {
            return $m[1] . str_replace(array("<", ">", "\n", "\r", "\t", " "), array("\1", "\2", "\3", "\4", "\5", "\7"), $m[3]) . $m[4];
        }, $t));
...
if(!function_exists('reorder')) {
    function reorder($id=null,$pos=null,$newpos=null) {
        if($pos!=$newpos) {
            if($newpos>$pos) {
                \DB::update("UPDATE tasks SET order_column=order_column-1 WHERE order_column<= '".$newpos."' AND id <>'".$id."'");
                \DB::update("UPDATE tasks SET order_column=order_column+1 WHERE order_column > '".$newpos."' AND id <>'".$id."'");
            } else {
                \DB::update("UPDATE tasks SET order_column=order_column-1 WHERE order_column < '".$newpos."' AND id <>'".$id."'");
                \DB::update("UPDATE tasks SET order_column=order_column+1 WHERE order_column >= '".$newpos."' AND id <>'".$id."'");
            ...
            \DB::update("UPDATE tasks SET order_column='".$newpos."' WHERE id ='".$id."'");
        }
    }
}
...
if(!function_exists('permissions')) {
    function permissions() {
        $permissions = [
            ...
            'attribute-email' => [
                'attribute-emails-list',
                'attribute-emails-create',
                'attribute-emails-edit',
                'attribute-emails-delete',
            ],
...
if(!function_exists('default_roles_with_permissions')) {
    function default_roles_with_permissions(){
        $default_roles_with_permissions = [
            'Full Administrator' => [
                'role-list',
                'role-create',
                'role-edit',
            ...
            return $default_roles_with_permissions;
    }
}


if(!function_exists('can_permission')) {
    function can_permission($permission,$target_title,$target_id) {
        if(auth()->user()->isAdmin() || auth()->user()->can('other-'.$target_title.'-'.$permission) || auth()->user()->id == $target_id) {
            return true;
        }
        return false;
    }
