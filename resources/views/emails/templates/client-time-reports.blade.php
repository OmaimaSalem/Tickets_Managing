                                ...
                                <table border="0" cellpadding="0" cellspacing="0"
                                    ...
                                    <tr>
                                        <td style="font-family: sans-serif; font-size: 14px; vertical-align: top;">
                                            ...
                                            @if($data['exceed'])
                                            {{ $data['time']->user->name }} has exceeded the limit:
                                            {{ $data['time']->time }}
                                            hours/{{ $data['time']->frequency }} - current time is
                                            :{{ $data['alltime']}} hour
                                            @else
                                            {{ $data['time']->user->name }} time:
                                            {{ $data['time']->time }}
                                            hours/{{ $data['time']->frequency }} - current time is
                                            :{{ $data['alltime']}} hour
                                            @endif
                                            ...
                                            Client tickets
                                            <ul>
                                                @foreach ($data['all_tickets'] as $ticket)
                                                <li><a target="_blank"
                                                        href="{{ url('/admin/ticket/'.$ticket->id) }}">{{ $ticket->name }}</a>
                                                </li>
                                                @endforeach
                                            </ul>
                                            @endif
                                            ...
                                            <ul>
                                                ...
                                                <li><a target="_blank"
                                                        href="{{ url('/admin/task/'.$task->id) }}">{{ $task->name }}</a>
                                                </li>
                                                @endforeach
                                            </ul>
                                            @endif
