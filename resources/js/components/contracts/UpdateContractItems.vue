    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="category">{{$t('ContractPage.category')}}</label>
                    ...
                    <div class="col-md-6">
                        <div v-if="!items">{{$t('ContractPage.selectCategory')}}</div>
                        <div v-else>
                            <table class="table table-hover table-sm">
                                <thead>
                                  <th>{{$t('ContractPage.name')}}</th>
                                  <th>{{$t('ContractPage.price')}}</th>
                                  <th>{{$t('ContractPage.desc')}}</th>
                                </thead>
                                <tbody>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-success" @click="add">{{$t('ContractPage.add')}}</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    created() {
        this.selectedItems = this.existItems; // Copy prop to local variable
    },
}
</script>
<style scoped>
