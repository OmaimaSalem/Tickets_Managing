        ...
        <table class="table table-hover table-sm">
          ...
          <tbody>
            <tr v-for="task in tasks" :key="task.id">
              ...
              <td>
                <div>{{ task.status.name | returnDoing }}</div>
              </td>
              <td>
        ...
        <div class="modal-content">
          ...
          >
            <div class="modal-body">
              ...
              <div class="form-group">
                <label for="estimated_time">{{ $t("Task.estimatedTime") }}</label>
                <input
...
<!--              >{{$t('Task.save')}}</button>-->
                <v-btn v-show="!editMode" type="submit" color="#ffffff" style="background-color:#234FA3 " text>{{$t('Task.save')}}</v-btn>
...
<!--              >{{$t('Task.update')}}</button>-->
                <v-btn v-show="editMode" type="submit" color="#ffffff" style="background-color:#234FA3 " text>{{$t('Task.update')}}</v-btn>
...
export default {
  ...
  methods: {
    ...
    getSingleProject() {
      ...
      this.isLoading = true;
      this.$store
        .dispatch("project/getProjectsByOwner", this.projectId)
        .then(response => {
