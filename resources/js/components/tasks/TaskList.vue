...
<!--          <button type="submit" style="background-color: #234FA3;color:#ffffff" class="btn btn-success btn-sm" @click="newModel">-->
<!--            <i class="fas fa-plus fa-fw"></i>-->
<!--            <span class="d-none d-lg-inline" style="font-weight: bold">{{$t('Task.newTask')}}</span>-->
<!--          </button>-->
            <v-btn
                color="#234FA3"
                dark
        ...
        <table class="table table-hover table-sm">
          ...
          <tbody>
            <tr v-for="task in tasks.data" :key="task.id">
              ...
              <td>
                <v-icon style="color:#43a355;font-size: 1rem;margin-left: .4rem" @click="editModel(task)">fas fa-edit</v-icon>
                <v-icon style="color:#C64848;font-size: 1rem;margin-left: .4rem" @click="deleteTask(task.id)">fas fa-trash</v-icon>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    getOwners() {
      this.$store
        .dispatch("owner/getOwners",{rules:false,dropdown:true})
        .then(response => {})
        .catch(error => {
          console.log(error);
        });
