<template>
	<div class="vlabeledit d-flex">
		...
		<textarea style="width: 100%;" v-if="edit" v-model="label" v-on:blur="updateTextBlur" ref="labeledit" :placeholder="vplaceholder" class="vlabeledit-input" @keyup.enter="updateTextEnter"></textarea>
	</div>
</template>
<script>
export default{
	...
	props: ['text', 'id', 'placeholder' ], // parent should provide :text or :placeholder
	methods: {
		initText: function(){
		...
		updateTextEnter: function(){
			this.edit = false;
			this.$emit('text-updated-enter',this.label , this.id )
		}
	},
	computed: {
