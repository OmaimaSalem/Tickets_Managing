    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">{{$t('Receipts.receiptsTable')}}</h3>

          <div class="card-tools">
            <button type="submit" class="btn btn-success btn-sm" @click="newModel">
              <i class="fas fa-plus fa-fw"></i>
              <span class="d-none d-lg-inline">{{$t('Receipts.newReceipt')}}</span>
            </button>
          </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body table-responsive p-0">
          <table class="table table-hover">
            <thead>
              <tr>
                <th width="10">ID</th>
                <th width="20%">{{$t('Receipts.Name')}}</th>
                <th width="40%">{{$t('Receipts.desc')}}</th>
                <th width="20%">{{$t('Receipts.task')}}</th>
                <th width="20%">{{$t('Receipts.total')}}</th>
                <th width="10%">{{$t('Receipts.isPaid')}}</th>
                <th>{{$t('Receipts.action')}}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="receipt in receipts" :key="receipt.id">
                ...
                <td>{{ receipt.total }}</td>
                <td v-if="!receipt.is_paid">{{$t('Receipts.notPaid')}}</td>
                <td v-else>{{$t('Receipts.paid')}}</td>
                <td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 v-show="!editMode" class="modal-title" id="newReceiptLabel">{{$t('Receipts.createNewReceipt')}}</h5>
            <h5 v-show="editMode" class="modal-title" id="newReceiptLabel">{{$t('Receipts.editReceipt')}}</h5>
          ...
          >
            <div class="modal-body">
              <div class="form-group">
                <label for="name">{{$t('Receipts.receiptName')}}</label>
                <input
                  v-model="form.name"
                  type="text"
                  name="name"
              ...
              <div class="form-group">
                <label for="description">{{$t('Receipts.receiptDesc')}}</label>
                <input
              ...
              <div class="form-group">
                <label for="name">{{$t('Receipts.task')}}</label>
                <multiselect
                  v-model="form.task"
                  :options="tasks.data"
                  ...
                  :placeholder="$t('Receipts.selectOne')"
                  label="name"
                ...
                >
                  <template slot="selection" slot-scope="{ values, search, isOpen }">
                    ...
                    >{{ values.length }} {{$t('Receipts.options')}}</span>
                  </template>
                </multiselect>
                <has-error :form="form" field="task_id"></has-error>
              </div>
              <div class="form-group">
                <label for="total">{{$t('Receipts.total')}}</label>
                <input
                  v-model="form.total"
                  type="text"
                  name="total"
              ...
              <div class="form-group">
                <div class="custom-control custom-switch">
                  <input
                    ...
                    id="is_paid"
                  />
                  <label class="custom-control-label" for="is_paid">{{$t('Receipts.isPaid')}}</label>
            ...
            <div class="modal-footer">
              <button v-show="!editMode" type="submit" class="btn btn-primary">{{$t('Receipts.save')}}</button>
              <button v-show="editMode" type="submit" class="btn btn-success">{{$t('Receipts.update')}}</button>
            </div>
          </form>
        </div>
      </div>
